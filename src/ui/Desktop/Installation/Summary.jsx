...
const Summary = () => {
  ...
  const { me, schemas, orgUnitGroupSets, orgUnitGroups, orgUnits, userGroups } = useMetadataStore(
    useShallow((state) => ({
      me: state.me,
  ...
  useEffect(() => {
    ...
    Object.keys(userGroupMapping).forEach((userGroupId) => {
      ...
      const groupMembers = JSON.parse(setupAuthorities[userGroupMapping[userGroupId]])
        .map((id) => {
          if (id.includes("userGroup")) {
            const foundUserGroup = userGroups.find((ug) => ug.id === id.replace("-userGroup", ""));
