...
const Accordion = ({
  ...
  return (
    <div className=" rounded-sm border border-gray-300 bg-white">
      {/* Header */}
      <button
        className={`flex w-full  sticky top-0 z-[10000] bg-neutral-200 items-center justify-between px-2 py-[5px]  text-left  ${
          open && "border-b border-gray-300 rounded-b-none mb-1"
        }`}
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
      >
        <span className={`text-base ${titleClassName}`}>{title}</span>
        <svg
      ...
      <div
        ...
        aria-hidden={!open}
      >
        <div className={`p-4 pt-0 text-sm text-gray-600 ${className}`}>
          {children}
        </div>
      </div>
    </div>
